...
\todo{Update for new constructors w/o of}
...
pattern or expression with its type using a colon \smlinline{:}. That type is then enforced by the compiler. Type annotations must not contain type variables (e.g., \smlinline{'a}).
\begin{smlcode}
val (x : int) = 1
...
for the whole \smlinline{let} expression. Only variables and functions can be
declared in a \smlinline{let} expression, not algebraic datatypes, exceptions,
or modules.
...
A \smlinline{raise} expression raises its argument as an exception. For example,\footnote{This example uses options (\S\ref{sect:option}) and TextIO (\S\ref{sect:textio}).}
\begin{smlcode}
fun checkPositive x =
...
\todo{Update once signatures are improved}.
